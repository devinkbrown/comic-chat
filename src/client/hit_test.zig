...
pub fn shell(layout: geometry.Layout, comic_mode: bool, x: i32, y: i32, member_count: usize) Target {
    ...
    if (contains(layout.toolbar, x, y)) return .{ .toolbar = toolbarIndex(x) orelse return .none };
...
fn menuIndex(pointer_x: i32) ?u8 {
    ...
    for (items, 0..) |item, index| {
        const right = x + Canvas.textWidth(item) + 18;
...
test "source shell hit targets distinguish controls and content" {
    ...
    try std.testing.expectEqual(Target{ .menu = 1 }, shell(layout, true, 61, 10, 3));
