...
| Message mutation and activity | `draft/message-redaction`, `draft/message-editing`, `draft/typing`, `draft/react`, `draft/reply`, `bot`, `draft/channel-context`, `draft/multiline` | Redaction/edit state, tags, multiline, and activity receive handling exist. `EDIT` and `REDACT` now have capability-gated outbound builders. Generic tagged reply/reaction/typing remains interoperable through `message-tags`; Onyx also accepts its narrower caps. |
