...
| History and conversations | `batch`, `draft/chathistory`, `draft/search`, `draft/event-playback`, `draft/read-marker`, `onyx/session-sync`, `onyx/bouncer`, `onyx/topics`, `draft/channel-rename` | BATCH/history/state processing exists. `SEARCH`, `MARKREAD`, and escaped `+onyx/topic` PRIVMSG/NOTICE builders are capability-gated. The three `onyx/*` server behaviors remain safe to negotiate without secret material. |
...
| `onyx/topics` | `@+onyx/topic=<escaped-label> PRIVMSG\|NOTICE <target> :<text>` |
| `draft/channel-context` + `message-tags` | `@+draft/channel-context=<channel> PRIVMSG <nick> :<text>` |
